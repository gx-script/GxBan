...
ScreenGui.Name = "GxUltimateV5_Confirmed"
...
local flying = false
createToggle("Smart Fly", function(val)
    flying = val
    if flying then
...
local autoAim = false
createToggle("Auto Aim Headshot", function(val)
    autoAim = val
    task.spawn(function()
        while autoAim do
            task.wait()
...
createToggle("Silent Aim Wallbang", function(val)
    silentAim = val
    local old; old = hookmetamethod(game, "__index", function(self, key)
        if silentAim and self == mouse and key == "Hit" then
            local target = nil
            local dist = math.huge
            for _, v in pairs(game.Players:GetPlayers()) do
